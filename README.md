...
ChatGPT Notebook is a Firefox and Chrome extension for saving useful ChatGPT messages into a per-conversation notebook.
...
ChatGPT Notebook is not available in Firefox Add-ons yet. Until then Firefox can only install it temporarily as a development/debug add-on. The Firefox instructions below are for this temporary setup.
...
ChatGPT Notebook is not available in the Chrome Web Store yet. For now, Chrome support is available as an unpacked development extension. A ready-to-load Chrome build is included at `extension/dist/chrome`.
...
5. Select this folder from the project:
   ...
   ```
6. Open ChatGPT, start or open a conversation, and click the ChatGPT Notebook toolbar icon. Chrome opens the extension in the browser side panel.
7. Use the **Export to ChatGPT Notebook** buttons on ChatGPT messages, or highlight text in a message and use the popover button, to save notes into the side panel.
...
If the ChatGPT page does not show **Export to ChatGPT Notebook** buttons, click the ChatGPT Notebook toolbar icon while the ChatGPT tab is active. If the buttons still do not appear, reload the ChatGPT tab once.
